...
log(process.argv);
const second_step = {};
[, , second_step.script_name] = process.argv;
if (second_step.script_name)
  second_step.callback = require(second_step.script_name);
log(second_step);
...
const _bitcoin_message_parser = (d) => {
  ...
  if (second_step.script_name)
    second_step.callback(payload, command, checksum, size);
};
